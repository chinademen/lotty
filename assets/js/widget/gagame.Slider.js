...
(function(host, name, Tab, $, undefined) {
	var defConfig = {
			...
			if (dire == "top") {
				...
				if (vi > len) {
					if (vi > length) {
						...
						me.getPanel(0).css({
							position: 'relative',
							left: length * cfg.sliderDistance
						});
					}
				}
				if (vi < 0) {
		...
		carouselAutoPlayGetIndex = function() {
			var me = this,
				length = me.length,
				len = length - 1,
				i = me._index + me.defConfig.autoPlayStep;
			me._index = i;
			return i;
		},
	...
	var pros = {
		init: function(cfg) {
			var me = this,
				cfg = me.defConfig,
				widtheight = cfg.sliderDirection == "left" ? "width" : "height";
		...
		show: function(i) {
			var me = this,
				cfg = me.defConfig,
				cls = cfg.currClass,
				step = cfg.sliderDistance,
				ti = me.getTriggerIndex(),
				pi = me.getPanelIndex(),
				dire = cfg.sliderDirection == "top" ? {
					"top": pi * step * -1
				} : {
					"left": pi * step * -1
				};

			me.sliderParent.animate(dire, {
				duration: cfg.sliderDuration,
				easing: cfg.sliderEasing
			});
	...
	var Main = host.ExtendClass(pros, Tab);
	Main.defConfig = defConfig;
	host[name] = Main;

})(gagame, "Slider", gagame.Tab, jQuery);