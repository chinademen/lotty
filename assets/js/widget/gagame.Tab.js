...
(function(host, name, Event, $, undefined) {
	var defConfig = {
		...
		autoPlayStart = function(e) {
			var me = e && e.data ? e.data : this,
				toEl = (e && e.relatedTarget) ? e.relatedTarget : false;
		...
		autoPlayGetIndex = function() {
			var me = this,
				length = me.length,
				len = length - 1,
				i = me.index + me.defConfig.autoPlayStep;
			/**
	...
	var pros = {
		init: function(cfg) {
			...
			if (cfg.isDefShow) {
				me._inits.push(function(cfg) {
					var me = this,
						i = cfg.isDefRandom ? parseInt(Math.random() * (me.length)) : cfg.index,
						cls = cfg.currClass,
						panel_cls = cfg.currPanelClass;
		...
		show: function(i) {
			var me = this,
				cls = me.defConfig.currClass,
				panelCls = me.defConfig.currPanelClass,
...
})(gagame, "Tab", gagame.Event, jQuery);